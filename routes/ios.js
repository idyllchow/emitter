...
router.get('/', function(req, res, next) {
  ...
  walk(iosPath, function(err, iosResults) {
    if (err) throw err;
      ...
      res.render('ios', {title:"iOS历史版本", archives:iosResults});
...
router.get('/:fileName', function(req, res, next) {
  ...
  if(fileName.indexOf('.apk') > -1) {
    tmp = '../public/archives/history/ios/';
  } 
    ...
    list.forEach(function(file) {
      ...
      fs.stat(file, function(err, stat) {
        if (stat && stat.isDirectory()) {
          ...
          var downloadUrl;
          // 修改时间, Date对象:
          if (path.basename(file).indexOf('release.ipa')!=-1) {
            type = 'release';
            platform = 'ios';
          ...
          } else if (path.basename(file).indexOf('debug.ipa')!=-1) {
            type = 'debug';
            platform = 'ios';
